...
namespace flowTools {
	...
	public:
		...
		void setup(int _width, int _height, ftForceType _type, bool _isTemporary) ;
		void applyForce(glm::vec2 _normalizedPosition);
		void update();
		void reset();
		
		void setName(string _name) {parameters.setName(_name);}
		void setType(ftForceType _type) { type = _type ;}
		...
		ftForceType getType() { return type ;}
	...
	protected:
		...
		void saveValue(ftForceType _type, glm::vec4 _force);
