...
namespace flowTools {
	...
	class ftParwShader : public ftShader {
	public:
		ftParwShader() {
			bInitialized = 1;
		...
		void glTwo() {
			fragmentShader = GLSL120(
									 ...
									 void main(){
										 ...
										 float sourceX = mix(sourceXUp, sourceXDown, pow(st.y, Power));
//										 float sourceX = mix(sourceXUp, sourceXDown, st.y);
										 ...
										 float sourceY = mix(sourceYLeft, sourceYRight, pow(st.x, Power));
//										 float sourceY = mix(sourceYLeft, sourceYRight, st.x);
										 
										 float upStrength = mix(Strength, Strength * 3.0, pow(st.x, Power));
										 
										 gl_FragColor = texture2DRect(AddTexture,vec2(sourceX * Scale.x, sourceY * Scale.y)) * vec4(vec3(upStrength), 1.0) + texture2DRect(Backbuffer, st2);
									 }
								  );
	...
	public:
		...
		void update(ofFbo& _buffer, ofTexture& _backBuffer, ofTexture& _texture, ofPoint _ul, ofPoint _ur, ofPoint _ll, ofPoint _lr, float _power, float _strength){
			_buffer.begin();
			shader.begin();
			shader.setUniformTexture("Backbuffer", _backBuffer, 0);
			shader.setUniformTexture("AddTexture", _texture, 1);
